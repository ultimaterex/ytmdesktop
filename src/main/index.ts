...
import { applyYtmProxyFromStore, attachProxyAuthHandler, getYtmPartitionName } from "./proxy/apply-ytm-proxy";
...
app.on("ready", async () => {
  ...
  store.onDidChange("proxy", async proxy => {
    await applyYtmProxyFromStore(store);
    if (ytmView) {
      ytmView.webContents.setWebRTCIPHandlingPolicy(proxy.enabled ? "disable_non_proxied_udp" : "default");
